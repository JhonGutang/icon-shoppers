...
const statuses = [
  ...
  { value: "DELIVERING", label: "Shipping", icon: Truck },
    ...
    <div className="flex h-screen flex-col bg-stone-50/50 overflow-hidden">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="hidden lg:flex w-80 border-r border-stone-200 bg-white p-8 flex-col gap-8">
          <div>
            <h1 className="text-3xl font-black text-stone-900 tracking-tight mb-2">My Orders</h1>
            <p className="text-stone-500 text-sm font-light">Track and manage your local purchases</p>
          </div>
          ...
          <nav className="flex flex-col gap-2">
            {statuses.map((s) => {
              ...
              return (
                ...
                >
                  <Icon size={18} className={cn(isActive ? "text-white" : "text-stone-400")} />
                  {s.label}
                  {isActive && (
                    <motion.div 
                      layoutId="activeTab"
          ...
          >
            <div className="max-w-4xl mx-auto space-y-8">
              <div className="lg:hidden mb-6">
                <h1 className="text-2xl font-black text-stone-900 tracking-tight">My Orders</h1>
                <p className="text-stone-500 text-xs font-light">Track and manage your local purchases</p>
              </div>
                  ...
                  {isLoading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-20 text-center">
                      ...
                      <p className="text-stone-500 font-light mt-2 max-w-xs">
                        It looks like there {activeTab === 'ALL' ? "aren't any orders" : `aren't any ${activeTab.toLowerCase()} orders`} yet. Keep exploring the local market!
                      </p>
                      <Button asChild variant="outline" className="mt-8 rounded-full px-8 hover:bg-stone-100">
                  ...
                  <div ref={observerTarget} className="py-8 flex justify-center w-full">
                    {isFetchingNextPage ? (
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <p className="text-sm font-bold text-stone-400 uppercase tracking-widest">Loading more orders...</p>
                      </div>
                    ) : hasNextPage ? (
                      <Button 
                        variant="ghost" 
                      ...
                      <div className="flex items-center gap-4 text-stone-300">
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-[0.2em]">End of list</span>
                        <div className="h-px w-12 bg-stone-200" />
                      </div>
                    )}
                  </div>
...
const OrderCard = ({ order }: { order: Order }) => {
  const getStatusStyle = (status: string) => {
    ...
    switch (s) {
      case 'ORDERED': return { bg: 'bg-orange-50', text: 'text-orange-600', border: 'border-orange-100', dot: 'bg-orange-400' };
      case 'APPROVED': return { bg: 'bg-blue-50', text: 'text-blue-600', border: 'border-blue-100', dot: 'bg-blue-400' };
      case 'PROCESSING': return { bg: 'bg-sky-50', text: 'text-sky-600', border: 'border-sky-100', dot: 'bg-sky-400' };
      case 'DELIVERING': return { bg: 'bg-indigo-50', text: 'text-indigo-600', border: 'border-indigo-100', dot: 'bg-indigo-400' };
      case 'DELIVERED': return { bg: 'bg-emerald-50', text: 'text-emerald-600', border: 'border-emerald-100', dot: 'bg-emerald-400' };
      case 'COMPLETED': return { bg: 'bg-[#0E6835]/10', text: 'text-[#0E6835]', border: 'border-[#0E6835]/20', dot: 'bg-[#0E6835]' };
      case 'CANCELLED': return { bg: 'bg-rose-50', text: 'text-rose-600', border: 'border-rose-100', dot: 'bg-rose-400' };
      default: return { bg: 'bg-stone-50', text: 'text-stone-600', border: 'border-stone-100', dot: 'bg-stone-400' };
    }
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between p-6 pb-4 gap-4 border-b border-stone-100 bg-stone-50/30">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-white border border-stone-200 flex items-center justify-center text-[#0E6835] shadow-sm overflow-hidden">
             {order.shop.logo ? (
                 <img src={`${process.env.NEXT_PUBLIC_LARAVEL_API_URL}/storage/${order.shop.logo}`} alt={order.shop.name} className="w-full h-full object-cover" />
             ) : (
                 <Store size={20} />
             )}
          </div>
          <div>
            <h4 className="font-black text-stone-900 group-hover:text-[#0E6835] transition-colors leading-none">{order.shop.name}</h4>
            <div className="flex items-center gap-2 mt-1.5">
              <span className="text-[10px] font-mono font-bold text-stone-400 tracking-wider">ORDER #{order.orderNumber}</span>
        ...
        <div className={cn("inline-flex items-center gap-2 px-4 py-2 rounded-full border text-[10px] font-black uppercase tracking-[0.2em]", style.bg, style.text, style.border)}>
        ...
        <div className="flex flex-col gap-6">
          <div className="space-y-4">
            {order.products.map((product: any) => (
              <div key={product.order_item_id} className="flex gap-4 group/item">
                ...
                <div className="flex-1 py-1 flex flex-col justify-between">
                  <div>
                    <h5 className="text-sm font-bold text-stone-900 group-hover/item:text-[#0E6835] transition-colors">{product.name}</h5>
                    <p className="text-xs text-stone-400 mt-0.5">Unit Price: ₱{product.price.toLocaleString()}</p>
                  </div>
                  <div className="flex items-center justify-between">
                    <p className="text-sm font-black text-stone-900">₱{(Number(product.price) * product.quantity).toLocaleString()}</p>
                    <Link href={`/products/${product.slug || '#'}`} className="text-[10px] font-bold text-[#0E6835] hover:underline flex items-center gap-1">
          ...
          <div className="flex flex-col sm:flex-row items-end sm:items-center justify-between gap-6">
            <div className="grid grid-cols-2 sm:flex items-center gap-6 w-full sm:w-auto">
              <div>
                <p className="text-[9px] font-bold text-stone-400 uppercase tracking-[0.15em] mb-1">Total Amount</p>
                <p className="text-2xl font-black text-[#0E6835] leading-none">₱{order.totalAmount.toLocaleString()}</p>
              </div>
              <div className="sm:pl-6 sm:border-l border-stone-100">
                <p className="text-[9px] font-bold text-stone-400 uppercase tracking-[0.15em] mb-1">Payment</p>
                <div className="flex flex-col">
                  <span className="text-xs font-bold text-stone-900 leading-none">{order.paymentMethod}</span>
                  <span className="text-[10px] text-stone-500 mt-1 uppercase">{order.paymentStatus}</span>
                </div>
              </div>
            </div>

            <div className="flex items-center gap-3 w-full sm:w-auto">
              <Button variant="outline" size="lg" asChild className="rounded-2xl flex-1 sm:flex-none h-12 px-8 border-stone-200 hover:bg-stone-50 group/btn">
                <Link href={`/orders/${order.orderNumber}`} className="flex items-center gap-2">
                  <span>Details</span>
              ...
              <AnimatePresence>
                ...
                {(order.status as string).toUpperCase() === 'DELIVERED' && (
                   <motion.div
                     ...
                     className="flex-1 sm:flex-none"
                   >
                     <Button className="w-full h-12 rounded-2xl bg-[#0E6835] hover:bg-green-800 text-white font-bold px-8 shadow-lg shadow-green-900/20">
                        Received
                     </Button>
                   </motion.div>
                )}
